...
public class CoursUiController {
    ...
    public String list(Model model,
        ...
        boolean filtreGratuit = gratuit != null && gratuit;
        Page<Cours> page;
        if (niveau != null && !niveau.isBlank()) {
            try {
                Cours.Niveau n = Cours.Niveau.valueOf(niveau);
                page = filtreGratuit
                        ? coursService.findByNiveauAndGratuit(n, pageable)
                        : coursService.findByNiveau(n, pageable);
                model.addAttribute("filtreNiveau", niveau);
            } catch (IllegalArgumentException ex) {
                page = filtreGratuit ? coursService.findGratuits(pageable) : coursService.findAll(pageable);
            }
        } else if (filtreGratuit) {
            page = coursService.findGratuits(pageable);
        } else {
            page = coursService.findAll(pageable);
        }
        if (filtreGratuit) {
            model.addAttribute("filtreGratuit", true);
        }
    ...
    public String details(@PathVariable Long id,
        ...
        Set<Long> completedLeconIds = progressionService.completedLeconIds(utilisateur.getId(), id);
        model.addAttribute("completedLeconIds", completedLeconIds);
        ...
        modules.forEach(m -> {
            ...
            quizService.findByModuleId(m.getId()).ifPresent(q -> {
                ...
                quizService.findLastResult(utilisateur.getId(), q.getId())
                        .ifPresent(r -> quizResults.put(q.getId(), r));
            });
        });
